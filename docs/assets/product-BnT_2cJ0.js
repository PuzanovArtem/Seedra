import"./header-BTWeqdbP.js";import"./search-z7pJi-Ut.js";const n="https://dummyjson.com/products/categories";async function d(){try{const i=await(await fetch(n)).json();m(i)}catch(s){console.error("Error fetching categories:",s)}}function m(s){const i=document.getElementById("catalog-category-list");s.forEach(a=>{const t=document.createElement("li");t.className="catalog__category-item catalog__category-btn",t.innerHTML=`
            <img src="/src/assets/images/icons/all-product-icon.svg" alt="${a.category}">
            <p>${a.name}</p>
        `,i.appendChild(t)})}d();const l="https://dummyjson.com/products?limit=9";console.log(l);async function g(){try{const a=(await(await fetch(l)).json()).products;p(a)}catch(s){console.log(s)}}function p(s){const i=document.getElementById("main-product__list");s.forEach(a=>{const t=document.createElement("li");t.classList="main-product__item",t.innerHTML=`<div class="main-product__heard-wrapper">
